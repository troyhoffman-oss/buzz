...
describe("clean source passes", () => {
  test(
    "the real src/ tree holds no protocol knowledge",
    () => {
      const result = Bun.spawnSync(["bash", SCRIPT], { cwd: CWD });
  ...
  test(
    "a same-prefixed unrelated package is not a protocol import",
    () => {
  ...
  test(
    "ordinary front-end code does not trip the gate",
    () => {
...
describe("violations are caught", () => {
  ...
  test(
    "a kind on a constant not named 'kind'",
    () => {
  ...
  test(
    "the kind-scan exemption is exact-path, not prefix or glob",
    () => {
  ...
  test(
    "a kind on a line after a URL literal",
    () => {
  ...
  test(
    "a protocol package imported without being declared",
    () => {
  ...
  test(
    "a protocol package imported by subpath",
    () => {
      expect(
        runWithProbe(
          "subpath.ts",
...
describe("comments do not trip the gate", () => {
  test(
    "a doc comment may explain the rule it documents",
    () => {
  ...
  test(
    "a line comment is stripped too",
    () => {
      expect(
