...
export function MembersSidebarMemberCard({
  ...
  const agentLocation = agentLocationLabel(managedAgent?.backend);
        ...
        {memberIsBot ? (
          <div className="relative min-w-0">
            <div className="flex min-w-0 items-center gap-2 transition-opacity duration-150 ease-out group-hover/member:opacity-0 group-focus-within/member:opacity-0">
              ...
              {/*
                Extends the existing bot metadata line rather than adding a
                row: same `text-xs text-muted-foreground` span the role uses.
                Provider-backed agents only — a local agent stays silent.
              */}
              {agentLocation ? (
                <span
                  className="inline-flex shrink-0 items-center text-xs text-muted-foreground"
                  data-testid={`sidebar-member-location-${member.pubkey}`}
                >
                  {agentLocation}
                </span>
              ) : null}
