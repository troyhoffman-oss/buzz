...
test("instance-edit carries a requested model prefill through", () => {
  ...
  // Same wrapping as the contract test above: unwrap the run-location
  // provider before asserting on the form's props.
  assert.equal(element.type, AgentRunLocationProvider);
  const form = element.props.children;
  assert.equal(form.type, AgentInstanceEditDialog);
  assert.equal(form.props.modelPrefill, "gpt-5-codex");
});
