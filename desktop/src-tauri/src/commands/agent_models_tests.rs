...
fn saved_agent_model_discovery_uses_record_snapshot() {
    ...
    // own key — resolved off the descriptor command, exactly as get_agent_models
    // does before calling effective_discovery_provider.
    assert_eq!(
        crate::managed_agents::known_acp_runtime(&descriptor.command)
            .and_then(|meta| meta.provider_env_var),
        Some("GOOSE_PROVIDER")
    );
}
